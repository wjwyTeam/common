...
//            date = fm1.format(unixLong);
	 ...
	 * @return
	 */
	public static String getOnYearString(String formatStr) {
		SimpleDateFormat format = new SimpleDateFormat(formatStr);
		Calendar c = Calendar.getInstance();
	 ...
	 * @return
	 */
	public static Integer getYearString() {
		Calendar cal=Calendar.getInstance();
	 ...
	 * @return
	 */
	public static Integer getMonthString() {
		Calendar c = Calendar.getInstance();
	 ...
	 * @return
	 */
	public static Integer getDayhString() {
		Calendar cal=Calendar.getInstance();
	 ...
	 * @param dataEnd 结束日期
	 * @param weekDay 获取周几，1－6代表周一到周六。0代表周日
	 * @return 返回日期List
	 */
	public static List<String> getDayOfWeekWithinDateInterval(String dataBegin, String dataEnd, int weekDay) {
	 ...
	 * @param year 开始日期
	 * @param month 结束日期
	 * @param index 第几周，从1，开始
	 * @param weekDay 获取周几，1－6代表周一到周六。0代表周日
	 * @return 返回日期List
	 */
	public static String getOneDate(String year, String month, int index, int weekDay) {
		String dataBegin = getFirstDayDate(year, month);
		String dataEnd = getLastDayDate(year, month);
		List<String> dateList = getDayOfWeekWithinDateInterval(dataBegin, dataEnd, weekDay);
		if(dateList.size() > index - 1) {
	...
	public static String getFirstDayDate(String year, String month) {
		if(month.length() == 1) {
			month = "0" + month;
		}
	...
	public static String getLastDayDate(String year, String month) {
		String firstDayDate = getFirstDayDate(year, month);
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		try {
			Date data = sdf.parse(firstDayDate);
			Calendar cal = Calendar.getInstance();
			cal.setTime(data);
			cal.set(Calendar.DAY_OF_MONTH, 1);
	...
	public static String getDisTimeStr(Date startDate, Date endDate) {
		...
		if (dis[0] != 0) {
			res.append(dis[0]).append("天前");
		} else if (dis[1] != 0) {
			res.append(dis[1]).append("小时前");
		} else if (dis[2] != 0) {
			res.append(dis[2]).append("分钟前");
		} else {
			res.append(dis[3]).append("秒前");
		}
		return res.toString();
	}
	 ...
	 * 得到当前月份字符串 格式（MM）
	 */
	public static String setMonth(Date T) {
		return formatDate(T, "MM");
	}
}
